    ...
    >
        <div class="mx-auto max-w-7xl px-6 lg:px-10">
            ...
            <nav class="border-b border-line py-5 text-xs uppercase tracking-luxe text-cream/70">
                ...
                <a href="{{ route('projects.index') }}" class="transition-colors hover:text-cream">Dự án</a>
                <span class="mx-2">/</span>
                <span class="text-cream">{{ $project->title }}</span>
            </nav>
            ...
            <div class="grid gap-10 py-12 lg:grid-cols-2 lg:gap-16">
                ...
                {{-- CỘT PHẢI: Thông tin --}}
                <div class="lg:py-2">
                    <h1 class="font-serif text-4xl font-light leading-tight tracking-wide text-cream lg:text-5xl">{{ $project->title }}</h1>

                    <p class="mt-4 text-xs uppercase tracking-luxe text-cream/70">
                        {{ collect([$project->location, $project->year_completed])->filter()->implode(' · ') }}
                    </p>
                    ...
                    <div class="mt-10">
                        ...
                        <dl class="mt-6 divide-y divide-line border-y border-line text-sm">
                            @if ($project->category)
                                <div class="flex gap-4 py-3"><dt class="w-36 shrink-0 font-medium text-cream">Loại công trình</dt><dd class="text-cream/70">{{ $project->category->name }}</dd></div>
                            @endif
                            @if ($project->location)
                                <div class="flex gap-4 py-3"><dt class="w-36 shrink-0 font-medium text-cream">Vị trí</dt><dd class="text-cream/70">{{ $project->location }}</dd></div>
                            @endif
                            @if ($project->area)
                                <div class="flex gap-4 py-3"><dt class="w-36 shrink-0 font-medium text-cream">Diện tích</dt><dd class="text-cream/70">{{ $project->area }}</dd></div>
                            @endif
                            @if ($project->year_completed)
                                <div class="flex gap-4 py-3"><dt class="w-36 shrink-0 font-medium text-cream">Năm hoàn thành</dt><dd class="text-cream/70">{{ $project->year_completed }}</dd></div>
                            @endif
                            @if ($project->style)
                                <div class="flex gap-4 py-3"><dt class="w-36 shrink-0 font-medium text-cream">Phong cách</dt><dd class="text-cream/70">{{ $project->style }}</dd></div>
                            @endif
                            @if ($project->client_name)
                                <div class="flex gap-4 py-3"><dt class="w-36 shrink-0 font-medium text-cream">Chủ đầu tư</dt><dd class="text-cream/70">{{ $project->client_name }}</dd></div>
                            @endif
                        </dl>
                    ...
                    @foreach ($related as $item)
                        <a href="{{ route('projects.show', $item) }}" class="group block">
                            <div class="aspect-[4/3] overflow-hidden bg-ink">
                                <img src="{{ Str::startsWith($item->grid_image, 'http') ? $item->grid_image : asset('storage/' . $item->grid_image) }}" alt="{{ $item->title }}" loading="lazy"
