{{-- DẢI CTA TƯ VẤN: nội dung lấy từ Admin → Cài đặt website → Dải kêu gọi (CTA) --}}
@php
    $c = $settings ?? [];
    $ctaImage = ($c['cta_image'] ?? '') ?: 'placeholders/project-5.jpg';
...
<section class="relative overflow-hidden">
    <div class="absolute inset-0 bg-cover bg-center bg-fixed" style="background-image:url('{{ asset('storage/' . $ctaImage) }}')"></div>
    ...
    <div class="reveal relative z-10 mx-auto flex max-w-3xl flex-col items-center px-6 py-28 text-center text-cream lg:py-40">
        <p class="text-xs font-medium uppercase tracking-luxe text-cream/60">{{ $ctaEyebrow }}</p>
        <h2 class="mt-5 font-serif text-4xl font-light leading-tight tracking-wide lg:text-5xl">{{ $ctaTitle }}</h2>
        <p class="mt-6 max-w-xl leading-relaxed text-cream/75">{{ $ctaSubtitle }}</p>
