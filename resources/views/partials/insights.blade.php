...
<section id="insights" class="bg-ink py-24 lg:py-32">
    <div class="mx-auto max-w-7xl px-6 lg:px-10">
        ...
        <div class="grid grid-cols-1 gap-x-8 gap-y-12 md:grid-cols-3">
            @foreach ($posts as $post)
                <a href="{{ route('blog.show', $post) }}" class="group reveal reveal-d{{ ($loop->index % 3) + 1 }} block">
                    ...
                    @endif
                    <span class="mt-4 inline-flex items-center gap-2 text-xs font-medium uppercase tracking-luxe text-cream">
                        Đọc tiếp
        ...
        <div class="reveal mt-16 text-center">
            <a href="{{ route('blog.index') }}" class="btn-line border-cream/70 text-cream hover:bg-cream hover:text-ink">
                Xem tất cả bài viết
