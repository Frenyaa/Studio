...
>
    <nav class="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-10">
        ...
        <a href="{{ route('home') }}" class="flex items-center gap-3 font-brand text-2xl font-semibold tracking-luxe drop-shadow-md">
            @if (!empty($siteLogo))
                <div class="bg-porcelain/95 px-3 py-1 rounded-sm shadow-sm flex items-center justify-center">
                    <img src="{{ asset('storage/' . $siteLogo) }}" alt="{{ $siteName ?? config('app.name') }}" class="h-16 w-auto object-contain">
                </div>
            @endif
    ...
    <div
        ...
        <ul class="mt-6 flex flex-col items-center gap-6 pb-16 font-serif text-3xl">
            ...
            <li class="flex flex-col items-center gap-2">
                ...
                <div class="flex max-w-xs flex-wrap justify-center gap-x-4 gap-y-1 font-sans text-xs uppercase tracking-luxe text-cream/50">
                    <a @click="mobileOpen = false" class="hover:text-accent" href="{{ route('about') }}#about">Về Studio</a>
