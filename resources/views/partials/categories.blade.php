...
<section id="categories" class="bg-cream py-24 lg:py-32" x-data="catSlider()">
        ...
        <div class="reveal mb-12 flex items-end justify-between gap-6">
            ...
            <div class="hidden shrink-0 gap-3 sm:flex">
                <button @click="scrollBy(-1)" aria-label="Trước"
                    class="flex h-12 w-12 items-center justify-center border border-ink/30 transition-colors duration-300 hover:bg-ink hover:text-cream">
            ...
            >
                <div class="relative overflow-hidden bg-ink">
                    <div class="aspect-[3/4] w-full overflow-hidden">
                        ...
                        @else
                            <div class="flex h-full w-full items-center justify-center bg-cream-deep text-ink-muted">
                ...
                <div class="mt-4 flex items-center justify-between">
                    <h3 class="font-serif text-2xl font-light text-ink">{{ $category->name }}</h3>
                    <svg class="h-5 w-5 -translate-x-2 text-ink opacity-0 transition-all duration-500 ease-luxe group-hover:translate-x-0 group-hover:opacity-100" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17.25 8.25 21 12m0 0-3.75 3.75M21 12H3"/></svg>
                </div>
            </a>
        @endforeach
    </div>
